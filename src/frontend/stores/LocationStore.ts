...
class LocationStore {
  ...
  @action async watchLocations() {
    ...
    for (let i = 0; i < len; i++) {
      ...
      const createdFiles = await Promise.all(
        createdPaths.map((path) => pathToFileDTO(path, location, this.rootStore.exifTool)),
      );
  ...
  @action.bound async initLocation(location: ClientLocation) {
    ...
    const files = await promiseAllLimit(
      filePaths.map((path) => () => pathToFileDTO(path, location, this.rootStore.exifTool)),
      N,
      showProgressToaster,
      () => isCancelled,
    );
  ...
  @action async addFile(fileStats: FileStats, location: ClientLocation) {
    ...
    // Gather file data
    const file = await pathToFileDTO(fileStats, location, this.rootStore.exifTool);
    ...
    if (match) {
      ...
      const newFileDTO = mergeMovedFile(dbMatch, file);
      this.rootStore.fileStore.save(newFileDTO);
    } else {
...
export async function pathToFileDTO(
  stats: FileStats,
  loc: ClientLocation,
  exifIO: ExifIO,
): Promise<FileDTO> {
