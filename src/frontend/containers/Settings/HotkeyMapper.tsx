...
import { Preferences, IHotkeyMap } from 'src/frontend/stores/Preferences';
...
export const HotkeyMapper = observer(() => {
  ...
  return (
    <div id="hotkey-mapper">
      {Object.entries(uiStore.preferences.hotkeyMap).map(([key, combo]) => {
...
const KeyComboEditor = ({
  ...
  const { uiStore } = useStore();
  const hotkeyMap = uiStore.preferences.hotkeyMap;
  const [text, setText] = textDispatch;
  const isEditable = useRef(true);
  const inputRef = useRef<HTMLInputElement>(null);
  const defaultCombo = Preferences.DEFAULT_HOTKEY_MAP[actionKey];
